...
# ---- tweak-data --------------------------------------------------------------
# Mark Twain Elementary in WA_duplicated_140
# Benson Hill Elementary in WA'
ds0 <- ds0 %>% 
  dplyr::mutate(
...
# ---- basic-graph --------------------------------------------------------------
# How many studetns in each school?
d1 <- ds0 %>% 
  dplyr::group_by(SchoolID) %>% 
  dplyr::summarize(
    n_students = length(unique(StudentID))
...
# ---- publish ---------------------------------------
path_report_1 <- "./analysis/0-eda-andriy/0-eda-andriy-0.Rmd"
